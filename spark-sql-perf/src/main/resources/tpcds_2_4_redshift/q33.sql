...
with ss as (
    ...
    group by i_manufact_id), cs as
         (select i_manufact_id, sum(cs_ext_sales_price) total_sales
          ...
          where
                  i_manufact_id in (
                  select i_manufact_id from dev.spectrum_iceberg_schema.store_returns.item
                  where
     ...
     ws as (
         ...
         from
             ...
             dev.spectrum_iceberg_schema.store_returns.item
         where
                 i_manufact_id in (select i_manufact_id from dev.spectrum_iceberg_schema.store_returns.item
                                   where i_category in ('Electronics'))
