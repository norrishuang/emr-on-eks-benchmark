--q23a.sql--
 ...
 (select substring(i_item_desc,1,30) itemdesc,i_item_sk item_sk,d_date solddate,count(*) cnt
  from  dev.{0}.store_sales,  dev.{0}.date_dim, dev.{0}.item
 ...
 (select max(csales) tpcds_cmax
  from (select c_customer_sk,sum(ss_quantity*ss_sales_price) csales
        from  dev.{0}.store_sales,  dev.{0}.customer,  dev.{0}.date_dim
 ...
 (select c_customer_sk,sum(ss_quantity*ss_sales_price) ssales
  from  dev.{0}.store_sales, dev.{0}.customer
  where ss_customer_sk = c_customer_sk
 ...
 from (select cs_quantity*cs_list_price sales
       from dev.{0}.catalog_sales,  dev.{0}.date_dim
       where d_year = 2000
         and d_moy = 2
      ...
      (select ws_quantity*ws_list_price sales
       from  dev.{0}.web_sales,  dev.{0}.date_dim
       where d_year = 2000
         and d_moy = 2
