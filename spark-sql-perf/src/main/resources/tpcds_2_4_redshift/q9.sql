...
select case when (select count(*) from dev.%s.store_sales
                  where ss_quantity between 1 and 20) > 74129
            then (select avg(ss_ext_discount_amt) from dev.%s.store_sales
                  where ss_quantity between 1 and 20)
            else (select avg(ss_net_paid) from dev.%s.store_sales
                  where ss_quantity between 1 and 20) end bucket1 ,
       case when (select count(*) from dev.%s.store_sales
